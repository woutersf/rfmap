...
function generateSnapshot(idx) {
  ...
  // ── DRONE SWARM ALPHA ─────────────────────────────────────────────────────
  // 12 drones moving NW→center. Last 25%: formation disperses, 5 are lost.
  const alphaLat = lerp(48.22, 48.06, t)
  const alphaLng = lerp(37.35, 37.75, t)
  ...
  // ── DRONE SWARM BETA ──────────────────────────────────────────────────────
  // 4 scout drones present for first 50% then destroyed/RTB
  if (t <= 0.5) {
    const bt = t / 0.5
    const bLat = lerp(48.05, 48.10, bt)
    const bLng = lerp(38.25, 37.95, bt)
    for (let i = 0; i < 4; i++) {
  ...
  const fLat = lerp(48.20, 48.04, t) - 0.04
  const fLng = lerp(37.33, 37.73, t) - 0.025
  for (let i = 0; i < 3; i++) {
  ...
  // ── WIFI CLUSTER 1 ────────────────────────────────────────────────────────
  // 8 devices near Donetsk east, small per-snapshot jitter
  for (let i = 0; i < 8; i++) {
    const rBase = createRng(i * 29 + 5000)
    const rJit = createRng(idx * 37 + i + 5000)
    const lat = 48.03 + (rBase() - 0.5) * 0.03 + (rJit() - 0.5) * 0.004
    const lng = 37.88 + (rBase() - 0.5) * 0.04 + (rJit() - 0.5) * 0.004
  ...
  // ── WIFI CLUSTER 2 ────────────────────────────────────────────────────────
  // 6 devices near Makiivka, small per-snapshot jitter
  for (let i = 0; i < 6; i++) {
    const rBase = createRng(i * 31 + 6000)
    const rJit = createRng(idx * 41 + i + 6000)
    const lat = 47.98 + (rBase() - 0.5) * 0.025 + (rJit() - 0.5) * 0.003
    const lng = 37.72 + (rBase() - 0.5) * 0.035 + (rJit() - 0.5) * 0.003
  ...
  // ── LONE SIGNALS ──────────────────────────────────────────────────────────
  const lone = [
    { id: 'uavx',  lat: 48.12, lng: 38.05, type: 'drone', label: 'UAV-X',  from: 0.45, to: 0.75 },
    { id: 'sig7',  lat: 47.95, lng: 38.05, type: 'radio', label: 'SIG-7',  from: 0.00, to: 0.40 },
    { id: 'dev3',  lat: 48.08, lng: 37.55, type: 'wifi',  label: 'DEV-3',  from: 0.60, to: 1.00 },
    { id: 'unk1',  lat: 48.18, lng: 38.10, type: 'drone', label: 'UNK-1',  from: 0.30, to: 0.60 },
  ]
